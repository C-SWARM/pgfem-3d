			     ...
			     MPI_Comm mpi_comm)
{
  ...
  int len = 500;
  char *line = PGFEM_calloc(len,sizeof(char));;
  ...
  /* allocate properties */
  (*props) = PGFEM_calloc(*n_mat,sizeof(cohesive_props));
  ...
  for(int i=0; i<*n_mat; i++){
	...
	p[i].nprops = 4;
	p[i].props = PGFEM_calloc(p[i].nprops,sizeof(double));
		       ...
		       const double *vars)
{
  ...
  double *Chi_s = PGFEM_calloc(ndim,sizeof(double));
  double *dChi_dX = PGFEM_calloc(ndim,sizeof(double));
  double *dChi_dN = PGFEM_calloc(ndim,sizeof(double));
  double *dChi_s_dX = PGFEM_calloc(ndim*ndim,sizeof(double));
  double *dChi_s_dN = PGFEM_calloc(ndim*ndim,sizeof(double));
