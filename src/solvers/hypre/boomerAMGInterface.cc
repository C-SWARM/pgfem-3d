...
enum relax_type {
  ...
  B_SOR,
  S_SOR,
  GAUSS_ELIM                                    /* coarsest level only */
};
...
enum smooth_type {
  SM_SCHWARZ=6, /* (default) Extra option routines:
                   HYPRE_BoomerAMG...
                   ...SetDomainType,
                   ...SetOverlap,
                   ...SetVariant,
...
enum schwarz_variant {
  SV_H_MULT, /* (default) no proc bnd overlap */
  SV_H_ADD, /* no proc bnd overlap */
  SV_ADDITIVE,
  SV_H_MULT_BND
};

enum schwarz_overlap {
  SOL_NONE,
  SOL_MIN, /* (default) */
  SOL_NEIGHBORS
};
...
int
pgfem3d::solvers::hypre::initializeBoomerAMG(HYPRE_Solver *hypre_pc,
...
void
pgfem3d::solvers::hypre::setBoomerAMGOptions(boomerAMGOptions *options)
{
