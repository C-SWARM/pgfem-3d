      ...
      prectype(opts.precond)
{
  ...
  Teuchos::RCP<const Teuchos::Comm<int> > mycomm = Teuchos::rcp(new Teuchos::MpiComm<int>(_comm));
  Teuchos::RCP<const Map> map = Teuchos::RCP<const Map>(new Map(Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid(),localrows,0,mycomm,node));
  const Teuchos::ArrayView<const int> colids(&Ai[Ap[0]],ourcols);

  Teuchos::RCP<const Map> colmap = Teuchos::RCP<const Map>(new Map(Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid(),colids,0,mycomm,node));
  

  const Teuchos::ArrayRCP< size_t > nCols(_nCols,0,localrows,false); 
  _localk=Teuchos::rcp(new TCrsMatrix(map,colmap,nCols,Tpetra::DynamicProfile));
...
TrilinosWrap::assemble()
{
  ...
  Tpetra::Export<int,int,Node> rowexporter(_localk->getRowMap(),_localk->getRowMap());
  const Teuchos::RCP<const TCrsMatrix> tempk = _localk;
  _k = Tpetra::exportAndFillCompleteCrsMatrix(tempk,rowexporter,_localk->getRowMap(),_localk->getRowMap());
}
...
TrilinosWrap::createSolver(int type, int maxit, double err, int kdim)
{
  ...
  int verbLevel = 0;
  //verbLevel+=Belos::Errors + Belos::Warnings + Belos::TimingDetails + Belos::FinalSummary + Belos::StatusTestDetails + Belos::Debug;
...
TrilinosWrap::createPreconditioner(const char *xmlpath)
{
  ...
  case PRECOND_MUELU: {
    ...
    _mueLuFactory = Teuchos::rcp(new MueLu::ParameterListInterpreter<ST,int,int,Node>(*_mueluList));
    _H = _mueLuFactory->CreateHierarchy();
    _H->IsPreconditioner(true);
  }
    break;
  ...
  case PRECOND_IFPACK2: {
    ...
    //Ifpack::Relaxation will complain if this is in the parameter list so remove it
                   ...
                   SOLVER_INFO *info)
{
  ...
  if(DomDof[rank] != (long)localLength) {
    std::cerr << "This won't work" << std::endl;
  }
                          ...
                          SOLVER_INFO *info)
{
  ...
  auto solution_1d = Kokkos::subview (solution_2d, Kokkos::ALL (), 0);
  _solution->modify<Kokkos::HostSpace>();
  //localLength = _solution->getLocalLength();
  if(DomDof[rank] != (long)localLength) {
    std::cerr << "This won't work" << std::endl;
  }
  for (size_t k = 0; k < localLength; ++k) {
...
TrilinosWrap::setupSolverEnv()
{
  ...
  switch (prectype) {
  case PRECOND_MUELU: {
    Teuchos::RCP<CrsMatrixClass> xmat_k = Teuchos::rcp(new MA(_k));
    Teuchos::RCP<MatrixClass> x_k = Teuchos::rcp(new CrsMatrixWrapClass(xmat_k));
    _H->GetLevel(0)->Set("A", x_k);
    _mueLuFactory->SetupHierarchy(*_H);  
    _preconditioner = Teuchos::rcp(new MueLu::TpetraOperator<ST,int,int,Node>(_H));
  }
    break;
  case PRECOND_IFPACK2: {
    // This dynamic casting shouldn't be necessary but the Ifpack2::Preconditioner base class does
    // not have member setMatrix(). The derived classes inherit this from
    // Ifpack2::Details::CanChangeMatrix. If other Ifpack2 preconditioners are used, they need
    // to be added here until a better fix is found (writing our own preconditioner wrapper?).
    if(_ifpacktype == "ILUT") {
      typedef Ifpack2::ILUT<TRowMatrix> prec_cast;
      (Teuchos::rcp_dynamic_cast<prec_cast, prec_type >(_ifpackprec))->setMatrix(_k.getConst());
    }
  ...
  _solver=Teuchos::rcp(new Belos::BlockGmresSolMgr<ST,MV,OP>( _problem, _belosList ));
...
TrilinosWrap::solve(SOLVER_INFO *info)
{
  ...
  if(ret == Belos::Unconverged) {
    std::cerr << "Trilinos did not converge with residual " << info->res_norm << std::endl;
  }
