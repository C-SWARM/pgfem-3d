                       ...
                       double *r_n_1)
{
 ...
 rest:
  ...
  while (STEP > DIV){
	...
	eps[0].F[1][1] = (1. - (times[tim] + (DIV+1)*dt)
			  /times[nt] * eps[0].load);
	eps[0].F[2][2] = 1.;
      }
      /* Simple shear */
	...
	eps[0].F[1][1] = 1.;
	eps[0].F[2][2] = 1.;
	eps[0].F[0][1] = (times[tim] + (DIV+1)*dt)/times[nt] * eps[0].load;
      }
      /* Deviatoric tension */
      if (eps[0].type == 3){
	eps[0].F[0][0] = 1./((1. - (times[tim] + (DIV+1)*dt)
			      /times[nt]* eps[0].load)
			     *(1. - (times[tim] + (DIV+1)*dt)
			       /times[nt] * eps[0].load));
	eps[0].F[1][1] = (1. - (times[tim] + (DIV+1)*dt)
			  /times[nt] * eps[0].load1);
	eps[0].F[2][2] = (1. - (times[tim] + (DIV+1)*dt)
			  /times[nt] * eps[0].load1);
      }
