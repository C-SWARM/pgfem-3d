				  ...
				  PGFEM_ms_job_intercomm **intercomm)
{
  ...
  *intercomm = NULL;
  ...
  /* working within the intercommunicator only */
    ...
    *intercomm = PGFEM_calloc(1,sizeof(PGFEM_ms_job_intercomm));
					 ...
					 PGFEM_ms_job_intercomm *ic)
{
  ...
  /* initialize receive */
  int *n_job_recv = PGFEM_calloc(nproc_inter,sizeof(int));

  /* post receives. NOTE: only commmunicate with macro processes which
     are listed first */
  MPI_Request *req_r = PGFEM_calloc(nproc_macro,sizeof(MPI_Request));
  int idx = 0;
  ...
  req_r = PGFEM_calloc(ic->recv_info->n_proc,sizeof(MPI_Request));
					 ...
					 MPI_Comm macro_comm)
{
  ...
  MPI_Request *req_s = PGFEM_calloc(nproc_inter-1,sizeof(MPI_Request));
  int idx = 0;
  ...
  if(ic->send_info->n_proc > 0){
    req_s = PGFEM_calloc(ic->send_info->n_proc,sizeof(MPI_Request));
