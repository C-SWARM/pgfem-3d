...
if ( $flag eq '-d' ) {
  ...
  $gridsize = shift;
  $t3d_options = "-d $gridsize -p 8 -k 2"; #'-\$ -X';
  $flag = shift;
}
...
if ( $flag eq 'clean') {
  system ("rm -rf $filebase.out *CPU partitions.* out");
  $flag = shift;
}
...
if ( $flag eq '-np' ) {
  ...
  while ( $nproc = shift ) {
    ...
    if ( $nproc != 1 ) {
      ...
      system ("cp box_QT_0.initial $dir/box_QT_0.initial");
	for ( $x = 1; $x < $nproc; $x = $x + 1 ) {
           system ("cp box_QT_1.initial $dir/box_QT_$x.initial"); 
           print "$x\n"
	}
    } else {
