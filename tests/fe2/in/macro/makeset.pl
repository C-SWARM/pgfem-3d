...
$filebase = "macro";
$ext = "st";
$t3d_options = "-d 10.0 -r 1 -p 8"; #'-\$ -X';
$decomposition_options = '-sp -rn';
$convert_options = '-co1 -v'; #'-pr 200.0 200.0 200.0';
...
if ( $flag eq '-np' ) {
  ...
  while ( $nproc = shift ) {
    ...
    if ( $nproc != 1 ) {
      system ("cp partitions.$nproc/$filebase.out.* $dir/.");
    } else {
...
cp -sf ../$filebase"."_0.in.co_props $filebase" . "_$i.in.co_props;");
    }
